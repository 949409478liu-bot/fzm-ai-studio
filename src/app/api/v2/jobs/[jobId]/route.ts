...
function jobDto(job: NonNullable<ReturnType<typeof getJob>>) {
  return { id: job.id, generationId: job.generationId, status: job.status, phase: job.phase, progress: job.progress, providerId: job.providerId, modelId: job.modelId, createdAt: job.createdAt, updatedAt: job.updatedAt, finishedAt: job.finishedAt, error: job.error };
}
