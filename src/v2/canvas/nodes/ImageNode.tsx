  ...
  return (
    <BaseNodeFrame title={data.title}>
      <div className="fzm-media-placeholder">
        {typeof data.assetId === "string" ? <img className="fzm-node-image" src={assetContentUrl(data.assetId, "thumbnail")} alt={data.originalName ?? "Image asset"} /> : <><ImageIcon size={24} /><span>Image shell</span></>}
      </div>
    </BaseNodeFrame>
  );
}
