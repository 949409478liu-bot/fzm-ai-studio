...
export function listJobs(projectId: string, options: { status?: string | null; limit?: number } = {}, db = getDb()) {
  ...
  return db.prepare(`SELECT * FROM jobs WHERE ${filters.join(" AND ")} ORDER BY created_at DESC, id DESC LIMIT ?`).all(...args).map((row) => mapJob(row as Record<string, unknown>));
}

export function insertJob(input: Omit<JobRecord, "id" | "remoteTaskId" | "status" | "phase" | "progress" | "attempt" | "nextRetryAt" | "ticket" | "error" | "createdAt" | "updatedAt" | "finishedAt">, db: Database.Database) {
  const id = randomUUID();
  const timestamp = now();
...
export function claimNextJob(db = getDb()) {
  const transaction = db.transaction(() => {
    const row = db.prepare("SELECT * FROM jobs WHERE status IN ('queued','rate_limited','provider_busy') AND (next_retry_at IS NULL OR next_retry_at <= ?) ORDER BY created_at ASC LIMIT 1").get(now()) as Record<string, unknown> | undefined;
    if (!row) return null;
    const result = db.prepare("UPDATE jobs SET status = 'preparing', phase = 'preparing', updated_at = ? WHERE id = ? AND status = ?").run(now(), row.id, row.status);
    return result.changes ? getJob(String(row.id), db) : null;
  });
  return transaction();
}

export function updateJob(id: string, patch: Partial<Pick<JobRecord, "status" | "phase" | "progress" | "attempt" | "nextRetryAt" | "remoteTaskId" | "ticket" | "error">>, db = getDb()) {
  ...
  db.prepare(`UPDATE jobs SET remote_task_id = ?, status = ?, phase = ?, progress = ?, attempt = ?, next_retry_at = ?, ticket_json = ?, error = ?, updated_at = ?, finished_at = ? WHERE id = ?`).run(next.remoteTaskId, next.status, next.phase, next.progress, next.attempt, next.nextRetryAt, JSON.stringify(next.ticket), next.error, now(), finished, id);
  return getJob(id, db);
}
...
export function recoverJobs(db = getDb()) {
  ...
  db.prepare("UPDATE jobs SET status = 'queued', phase = 'queued', updated_at = ? WHERE status = 'downloading' OR status = 'finalizing'").run(now());
}

export function cancelJob(id: string, db = getDb()) { return updateJob(id, { status: "canceled", phase: "canceled" }, db); }
