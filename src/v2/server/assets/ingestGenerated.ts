...
import type { FetchedProviderOutput } from "@/v2/server/providers/types";
...
export async function ingestGeneratedOutput(input: {
  projectId: string;
  generationId: string;
  jobId: string;
  providerId: string;
  modelId: string;
  output: FetchedProviderOutput;
}): Promise<V2Asset> {
  assetRoot();
  const temp = tempPath(`${randomUUID()}.generated`);
  let movedOriginal: string | null = null;
  let createdThumbnail: string | null = null;
  let targetPreExisted = false;
  try {
    await fs.writeFile(temp, input.output.bytes, { flag: "wx" });
    ...
    const existing = findProjectAssetBySha(input.projectId, sha256);
    if (existing) {
    ...
    return insertAsset({
      ...
      originalName: input.output.suggestedName || null,
      byteSize: input.output.bytes.byteLength,
      width,
      height,
      duration: null,
      sha256,
      thumbnailPath,
      source: "generated",
      metadata: { generationId: input.generationId, jobId: input.jobId, providerId: input.providerId, modelId: input.modelId, ...(input.output.metadata ?? {}) },
    });
  } catch (error) {
    await removeIfExists(temp);
